...
class NotesMod(loader.Module):
	...
	async def savecmd(self, message):
		...
		self._db.set("friendly-telegram.modules.notes", "notes",
		             {**self._db.get("friendly-telegram.modules.notes", "notes", {}), args[0]: asset_id})
	...
	async def notescmd(self, message):
		...
		if not self._db.get("friendly-telegram.modules.notes", "notes", {}):
			...
			return
		pref=self._db.get('friendly-telegram.main', 'command_prefix', ['.'])[0]
		await utils.answer(message, self.strings("notes_header", message) + "\n".join(f"<code>{pref}note {key}</code>" for key in self._db.get("friendly-telegram.modules.notes", "notes", {})))
